...
def bar_rows(slices: list[Slice], *, max_value: float | None = None, unit: str = "") -> str:
    ...
    for s in present:
        ...
        rows.append(
            ...
            '<div class="row-track">'
            f'<div class="row-fill" style="width:{max(width, 0.6):.4f}%;background:{s.colour}" '
...
def funnel_rows(slices: list[Slice]) -> str:
    """A nested-subset funnel: descending bars, one hue, each labelled with its share."""
    if not slices or slices[0].value <= 0:
        return '<p class="empty">No clauses to classify.</p>'
    top = slices[0].value
    rows = []
    for s in slices:
        width = _pct(s.value, top)
        rows.append(
            '<div class="row">'
            f'<div class="row-label" title="{escape(s.note or s.label)}">{escape(s.label)}</div>'
            '<div class="row-track">'
            f'<div class="row-fill" style="width:{max(width, 0.6):.4f}%;background:{s.colour}" '
            f'title="{escape(s.label)}: {s.value:,.0f} of {top:,.0f}"></div>'
            "</div>"
            f'<div class="row-value">{s.value:,.0f}'
            f'<span class="row-pct">{width:.0f}%</span></div>'
            "</div>"
        )
    return f'<div class="bars funnel">{"".join(rows)}</div>'
