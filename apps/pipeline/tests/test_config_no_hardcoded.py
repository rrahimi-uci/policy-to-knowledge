...
class TestConfigGetters:
    ...
    def test_get_document_workers(self):
        assert get_config().get_document_workers() == 6

    def test_get_optimized_performance_profile(self):
        profile = get_config().get_performance_profile()
        assert profile["global_llm_concurrency_initial"] == 12
        assert profile["global_llm_concurrency_max"] == 32
        assert profile["llm_concurrency"] == 16
        ...
        assert profile["grounding_llm_concurrency"] == 24
